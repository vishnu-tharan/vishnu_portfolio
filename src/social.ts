...
export function whatsappUrl() {
  const msg = encodeURIComponent(`Hi ${profile.shortName}, I found your portfolio and would like to connect!`)
  return `https://wa.me/${profile.whatsapp}?text=${msg}`
}

const socialLinks = [
  { id: 'linkedin', href: profile.linkedin, label: 'LinkedIn', icon: linkedinSvg, class: 'social-linkedin' },
  { id: 'whatsapp', href: whatsappUrl(), label: 'WhatsApp', icon: whatsappSvg, class: 'social-whatsapp' },
  { id: 'instagram', href: profile.instagram, label: 'Instagram', icon: instagramSvg, class: 'social-instagram' },
]
...
export function renderSocialIcons() {
  return socialLinks
    .map(
      (link) =>
        `<a href="${link.href}" target="_blank" rel="noopener noreferrer" class="social-icon ${link.class}" aria-label="${link.label}">${link.icon}</a>`
    )
    .join('')
}

export function renderSocialButtons() {
  return `<div class="social-buttons">${socialLinks
    .map(
      (link) =>
        `<a href="${link.href}" target="_blank" rel="noopener noreferrer" class="social-btn ${link.class}">${link.icon}<span>${link.label}</span></a>`
    )
    .join('')}</div>`
}
